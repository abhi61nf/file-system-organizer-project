...
function organizeFn(dirpath) {
    ...
    if (dirpath == undefined) {
      ...
      if (doesExist == true) {
        ...
        //   C:\Users\abhinav goel\OneDrive\Desktop\file system organizer project\test folder - I want to create a folder in this path
    ...
    organizeHelper(dirpath, destPath);   // we are writing this function to categorize our files
  }
  ...
  function organizeHelper(src, dest) {
    ...
    for (let i = 0; i < childNames.length; i++) {
      let childAddress = path.join(src, childNames[i]); // path is identified for the files , PATH.JOIN path nikal deta hai 
      let isFile = fs.lstatSync(childAddress).isFile(); // "lstatSync" checks whether it is file or not , i.e returns either true or false 
